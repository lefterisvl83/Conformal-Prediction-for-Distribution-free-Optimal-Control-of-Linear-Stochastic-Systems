...
ellipsoid_points=draw_ellipsis(Y);
figure(1);
hold on
plot(ellipsoid_points(1, :), ellipsoid_points(2, :),'Color', 'green', 'LineWidth', 2, 'LineStyle', '--');
% for i=1:M
%     plot(x{i}(1,:),x{i}(2,:),'*')
% end
ellipsoid_points=draw_ellipsis(Yel);
plot(ellipsoid_points(1, :), ellipsoid_points(2, :), 'Color', [.5 .5 .5], 'LineWidth', 2, 'LineStyle', '--');
