...
B = kron(eye(6),[0; 0.5]);
Pt = kron(eye(6),(1/10)*eye(2));
Y = kron(eye(6), [12.6733, -1.0720;-1.0720, 114.7949]); % disturbance matrix
eps=1e-6;
Q = kron(eye(6),1);
% solve (22)
step_size=0.05 ;
...
for i=1:MM
    ...
    Psi = sdpvar(6,12);
    F = [];
    F = [Phihat <= inv(Pt)+eye(12)*eps];
    F = [F, [Phihat-(1/lambda1_vals(i))*inv(Y), A*Phihat+B*Psi; (A*Phihat+B*Psi)', lambda0_vals(i)*Phihat]>=0];
    F = [F, [Phihat, Psi'*sqrt(Q); sqrt(Q)*Psi, (1+eps)*eye(6)]>=0];
    F = [F, Phihat<=inv(Pt)-eps*eye(12)];
    F = [F, Phihat>=0];
